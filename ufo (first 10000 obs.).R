...
dataCleaned %>%
  filter(nchar(location) < 8)  #--> unnormal
...
locatNA <- data.frame(
  rownum = 
    which( #nchar(dataCleaned$location) < 8 |
           nchar(dataCleaned$location) > 100  )
 )
locatNA[2:4] <- dataCleaned[locatNA$rownum , ]
locatNA[ , 4] <- NA
dataCleaned[locatNA$rownum , 3] <- locatNA$location
...
for(i in 1:length(x)){
  ...
  ufoSight[,4][i] <- x[[i]][2]
    ...
    ufoSight[,3][i] <- paste0(x[[i]][1:(n-1)] , collapse = "")
    ufoSight[,4][i] <- x[[i]][n] 
  }
}
rm(i,x,n)
...
stateNA <- data.frame(
  rownum = 
    which( nchar(ufoSight$state) != 3
         )
 )
stateNA[2:5] <- ufoSight[stateNA$rownum , ]

stateNA %>%
  arrange(nchar(state)) -> stateNA
...
ufoSight %>%
  filter( state %in% state50 ) ->ufoSight 

date = seq(
        min(ufoSight$date , na.rm = T),
        max(ufoSight$date , na.rm = T),
        by = "1 month" )
...
ggplot(filter(dtForPic , date >= "1980-01-02" && date <= "2001-01-02"))+
  geom_line(aes(date,numOfSight) , color = "darkblue" )+
  facet_wrap(~ state , nrow = 10)+
  #scale_x_discrete(breaks = c("1994-08","1994-12","1995-08"))+
  labs(list(
    title = "Number of UFO sightings by Month???Year and U.S. State (1980???2001)",
